...
class HabboParser implements HabboParserInterface
{
	...
	{
		$redirect[ 'url' ]	 = '/gamedata/furnidata/1';
		$redirect[ 'data' ]	 = $this->_callUrl($this->api_base.$redirect[ 'url' ]);
		$url				 = $redirect[ 'data' ][ 1 ][ 'redirect_url' ];

		list($data) = $this->_callUrl($url, true);
		$furnis = array();

		foreach($data as $furni_data)
		{
			$temp_furni	 = new Furni();
			$temp_furni->parse($furni_data);
			$furnis[]	 = $temp_furni;
			unset($temp_furni);
		}
	...
	protected function _callUrl($url, $furniCall = false)
	{
		...
		if($furniCall)
		{
			$response = $this->furniResponse($data);
		}else
		{
